...
export const SolutionPopup = props => {
    console.log("WiziQuestion SolutionPopup props", props);
    ...
    const fetchSolution = () => {
        ...
        apiClient.fetchSolution(props.record.question_img_url, suggestion)
            .then((res) => {
                ...
                setSolution(res);
                // Update wizi_question solution instead of question
                apiClient.updateWiziQuestionSolution(
                    props.record.id,
                    res).then(() => {
                        console.log("Solution updated in wizi_question record");
                    }).catch((err) => {
                        console.error("Error updating wizi_question solution:", err);
                    });
