    ...
    login: async ({ username, password }) => {
        try {
            ...
            if (response.status === 'success' && response.result) {
                ...
                localStorage.setItem('display_name', response.result.display_name || username);
                localStorage.setItem('role_id', response.result.role_id?.toString() || '');
                ...
                return Promise.reject(new Error(response.message || 'Login failed'));
            }
        } catch (error) {
            return Promise.reject(new Error(error.message || 'Network error during login'));
        }
    },
    logout: () => {
